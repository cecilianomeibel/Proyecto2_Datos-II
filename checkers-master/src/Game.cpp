...
sf::Time Game::timeElapsed;
int Game::winner = 0; // no hay ganador en la inicialización

Game::Game(sf::RenderWindow &window, const bool &isHuman1, const bool &isHuman2) {
    isPlaying = false; // bucle de juego booleano
    checkerboard = new Tablero(window); // crea un nuevo tablero de ajedrez
    //música para los movimientos
    Sonido::getSoundManager()->addSound("../resources/bites-ta-da-winner.ogg");
    Sonido::getSoundManager()->addSound("../resources/ganar-ganador.ogg");
    Sonido::getSoundManager()->addSound("../resources/mario-bros-woo-hoo.ogg");
    Sonido::getSoundManager()->addSound("../resources/bonus.ogg");
    p1 = new Jugador(1, isHuman1); // create player 1
    p2 = new Jugador(2, isHuman2); // create player 2
}
...
void Game::startCheckers(sf::RenderWindow &window, sf::Event &event) {
...
void Game::gameLoop(sf::RenderWindow &window, sf::Event &event) {
    ...
    Cuadrado *currentSquare = nullptr; // el cuadrado actual seleccionado
    Cuadrado *futureSquare = nullptr; // el futuro cuadrado seleccionado
    Cuadrado *tempSquare = nullptr; // el cuadrado temporal puede cumplir múltiples roles, consulte los métodos jumpBySquare / jumpByChecker en la clase Movimiento.
...
/************************************************************* AI MOVEMENTS ******************************************/
        if (p2->getTurn() && !p2->getIsHuman()) // el jugador 2 es AI y este es su turno
        {
            ...
            if (currentIndex ==
                -1) // tenemos que hacer esto antes de establecer las coordenadas (no se pueden establecer las cuerdas en nullptr)
            {
            ...
            } else // las coordenadas serán de tamaño 4 o 6
            {
                ...
                if (coords.size() == 4) // only a move
                {
                    ...
                    if (currentSquare != nullptr && futureSquare != nullptr) {
                        ...
                        p2->getChecker(currentIndex)->setPosition((float) coords[2],
                                                                  (float) coords[3]); // actualiza la posición actual de la ficha
                        ...
                        selecting = true;
                        Sonido::getSoundManager()->playSound(SOUND_MOVE);
                    }
                } else if (coords.size() == 6) // jumpByChecker
                {
                    ...
                    if (currentSquare != nullptr && futureSquare != nullptr && tempSquare != nullptr) {
                        ...
                        p2->getChecker(currentIndex)->setPosition((float) coords[4],
                                                                  (float) coords[5]); // actualiza la posición actual de la ficha
                        // comprobar si nos coronaron
                        ...
                        changeTurn(); // change to player 1 turn
                        selecting = true;
                        Sonido::getSoundManager()->playSound(SOUND_JUMP_CHECKER);
                    }
                }
            }
                        ...
                        if (currentSquare != nullptr && currentSquare->getOccupied()) // validar selección
                        {
                            if (p1->getTurn()) // p1's turn
                            {
                                ...
                                if (currentIndex != -1 && currentIndex < p1->getCounter()) {
                                    ...
                                    if (Movimiento::hasJump(p1->getChecker(currentIndex), p2->getCheckersVector(),
                                                            checkerboard)) {
                            ...
                            } else // turno de p2
                            {
                                ...
                                if (currentIndex != -1 && currentIndex < p2->getCounter()) {
                                    ...
                                    if (Movimiento::hasJump(p2->getChecker(currentIndex), p1->getCheckersVector(),
                                                            checkerboard)) {
                                        selecting = false;
                                    } else {
...
/**************************************************** END OF PLAYERS SELECTING. *****************************************************************************/
                    } else if (!selecting) // una pieza de ajedrez se mueve o salta
                    {
                        ...
                        futureSquare = checkerboard->findSquare(futureX,
                                                                futureY); // El cuadrado al que el jugador quiere moverse
                        generalDirection = Movimiento::findGeneralDirection(currentSquare,
                                                                            futureSquare); // encuentra la dirección general (en grados) con respecto al futuro cuadrado seleccionado
                        ...
                        if (generalDirection == -1) {
                            ...
                            std::cout << "The Future Cuadrado was not found." << std::endl;

/************************************************************* CODE FOR BOTH PLAYERS MOVEMENTS. *******************************************************/
                        if (tempSquare !=
                            nullptr) // la única forma en que tempSquare es nulo es cuando no se encuentra generalDirection == -1 o futureSquare.
                        {
                            if (futureSquare->getFillColor() ==
                                sf::Color(92, 37, 0)) //Color cafe // make sure the future square is moveable
                            {
                                if (p1->getTurn()) {
                                    if (p1->getIsHuman()) {
                                        ...
                                        if (Movimiento::jumpByChecker(p1->getCheckersVector(), currentSquare,
                                        ...
                                        else if (Movimiento::jumpBySquare(p1->getCheckersVector(), currentSquare,
                                                                          futureSquare, tempSquare, currentIndex)) {
                                            ...
                                            checkDoubleJump = true; // acabamos de completar un salto, veamos si hay un doble salto disponible
                                            changeTurn();
                                            Sonido::getSoundManager()->playSound(SOUND_JUMP_SQUARE);
                                        }
                                            // finalmente verifique si se mueve sin saltar
                                        else if (
                                                Movimiento::moveable(p1->getCheckersVector(), currentSquare,
                                                                     futureSquare,
                                            ...
                                            if (!Movimiento::hasJump(p1->getChecker(currentIndex),
                                        ...
                                        if (checkDoubleJump &&
                                            Movimiento::hasJump(p1->getChecker(currentIndex), p2->getCheckersVector(),
                                                                checkerboard)) {
                                ...
                                {
                                    if (p2->getIsHuman()) {
                                        if (Movimiento::jumpByChecker(p2->getCheckersVector(), currentSquare,
                                                                      futureSquare, tempSquare, currentIndex)) {
                                            currentSquare->setOccupied(false);
                                            futureSquare->setOccupied(false);
                                            tempSquare->setOccupied(true);
                                            ...
                                            changeTurn();
                                            Sonido::getSoundManager()->playSound(SOUND_JUMP_CHECKER);
                                        } else if (Movimiento::jumpBySquare(p2->getCheckersVector(), currentSquare,
                                                                            futureSquare, tempSquare, currentIndex)) {
                                            futureSquare->setOccupied(true);
                                            currentSquare->setOccupied(false);
                                            ...
                                            Sonido::getSoundManager()->playSound(SOUND_JUMP_SQUARE);
                                                ...
                                                Movimiento::moveable(p2->getCheckersVector(), currentSquare,
                                                                     futureSquare,
                                                                     currentIndex) && !futureSquare->getOccupied()) {
                                            if (!Movimiento::hasJump(p2->getChecker(currentIndex),
                                        ...
                                        if (checkDoubleJump &&
                                            Movimiento::hasJump(p2->getChecker(currentIndex), p1->getCheckersVector(),
                                                                checkerboard)) {
    ...
    for (int i = 0; i < SQUARES_VERTICAL; ++i) {
        for (int j = 0; j < SQUARES_HORIZONTAL; ++j) {
            ...
            if (checkerboard->findSquare(idNumber++)->getOccupied()) {
                ...
                if (i <= 2)
                    p1->addChecker(new PiezaDama(sf::Color::White, radius, startingX, startingY, KING_ROW_7, i,
                                                 p1->getNumber()));
                else
                    p2->addChecker(new PiezaDama(sf::Color::Black, radius, startingX, startingY, KING_ROW_0, i,
                                                 p2->getNumber()));
            }
            startingX += XOFFSET;
        }
...
bool Game::playerHasToJump(Jugador *&active, Jugador *&notActive) {
    ...
    for (int i = 0; i < active->getCounter(); ++i)
        if (Movimiento::hasJump(active->getChecker(i), notActive->getCheckersVector(), checkerboard))
...
*/
bool Game::playerCannotMove(Jugador *&p1, Jugador *&p2, Tablero *&checkerboard) {
    if (p1->getTurn() == true) {
        AI ai;
...
void Game::handleGameState(sf::Clock &clock) {
    // tied
    if (p1->getCounter() == p2->getCounter()) {
        ...
        winner = p2->getNumber();
        Game::timeElapsed = clock.restart(); // obtener la duración del juego
        std::cout << "Time Elapsed (in minutes): " << (Game::timeElapsed.asSeconds() / 60) << std::endl;
        saveTime((Game::timeElapsed.asSeconds() / 60)); // guarda el tiempo en el almacenamiento externo
    }
        // player 1 wins
...
void Game::printChecker(PiezaDama *selected, const std::string &action) {
...
void Game::deleteCheckerFromGame(Jugador *deleteFrom, const int &deleteIndex) {
...
void Game::ifCheckerKinged(PiezaDama *checker, Cuadrado *square) {
    if ((square->getRow() == KING_ROW_0 && checker->getKingRow() == KING_ROW_0)
        ...
        Sonido::getSoundManager()->playSound(SOUND_KING);
    }
}

// save the time
void Game::saveTime(const double &time) {
...
void Game::showWinner(sf::RenderWindow &window, sf::Event &event) {
