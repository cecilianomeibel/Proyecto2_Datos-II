...
*	Each team starts with 12 checkerpieces. This class handles drawing to checkerboard (called within Game.cpp),
...
*	The actual checkerpieces are managed in the Jugador class (each player has a vector container).
*/
...
class PiezaDama : public sf::CircleShape {
private:
protected:
    ...
    bool isKing;
public:
    PiezaDama(); // dummy constructor for drawing object in Game class
    PiezaDama(sf::Color, int, int, int, const int, const int, const int);

    void drawCheckers(sf::RenderWindow &, const std::vector<PiezaDama *> &);
